...
def threeaddr_to_spim(three_addr_code,main_proc_name):
	...
	for instr in three_addr_code:
		...
		if op=='label':
			...
			if op=='+':
				instr_code+='\tadd $t2, $t0, $t1\n'
			elif op=='/':
				instr_code += '\tdiv $t2, $t0, $t1\n'
			elif op=='*':
				instr_code += '\tmul $t2, $t0, $t1\n'
			elif op== '-':
				instr_code += '\tsub $t2, $t0, $t1\n'
			
			instr_code += StoreInstr(instr.result, 't2', current_env)
		...
		elif op=='unary+' and instr.arg2==None:
			print 'Unary plus'
			instr_code += LoadInstr(instr.arg1, 't0', current_env)
			instr_code += StoreInstr(instr.result, 't0', current_env)
			...
			print 'unary minus'
			instr_code += LoadInstr(instr.arg1, 't1', current_env)
			instr_code += '\tneg $t0, $t1\n' 
			instr_code += StoreInstr(instr.result, 't0', current_env)
...
def LoadInstr(x,reg,env):
	if type(x)==type(0):			
		return '\tli $' + reg + ', ' + str(x) + '\n'
	elif type(x)==type(0.0):
		return '\tli.s $' + reg + ', ' + str(x) + '\n'
	elif '\'' in x:
		return '\tli $' + reg + ', ' + x + '\n'
	elif '(' in x :
		y=x.replace('(',',').replace(')','').split(',')
		ans=LoadInstr(env.get_attribute_value(y[0],'offset'),'t5',env)
		for i in range(1,len(y)):
			if (env.is_present(y[i])):
				ans+=LoadInstr(y[1],'t4',env)
			else :
				ans+=LoadInstr(int(float(y[1])),'t4',env)
